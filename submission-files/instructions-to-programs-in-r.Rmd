...
header-includes:
  ...
  \rhead{Novo Nordisk - \today} \lhead{BLA 761326 - Insulin icodec}
  \rfoot{\thepage} 
...
![](./inst/figures/pkglite-build.png)
...
![](./inst/figures/build-projects.png)

The folder specified in the `pkglite_path` now looks like this:

![](./inst/figures/folder-project-built.png)

with the following subfolders - one for each trial:

![](./inst/figures/view-built-trial-folders.png)

Open the `program` folder e.g. `nn1436/4477/custom/stats/program` and check that the project files are there:

![](./inst/figures/unpacked-prog-folder.png)
...
To restore the environment first open the `program.Rproj` file. This should open up Rstudio. As `renv` is bootstrapping itself you might see this screen for ~10 seconds:

![](./inst/figures/rstudio-open-renv.png)

Followed by:

![](./inst/figures/renv-start.png)
...
![](./inst/figures/lib-NNBiostat.png)
...
Once the environment is set up data can be copied to the `stats/data` folder e.g. ADaM data in `nn1436/4477/custom/stats/data/adam` like so:

![](./inst/figures/data.png)

R programs for tables, listings and figures should be copied to the `stats/program/statprog` folder e.g. `nn1436/4477/custom/stats/program/statprog`. Note that if the programs copied into the instance have the `.txt` extension then this should be changed back to `.r/.R` in order for Rstudio to process it as a script.

![](./inst/figures/rename-to-r.png)
...
To make this function work in the local environment the `nnaccess()` call should be modified as follows for e.g. `4477`:

```{r, eval=FALSE}
library(NNBiostat)

(db <- nnaccess(trial = "4477",
                project = "nn1436",
                instance = "custom",
...
![](./inst/figures/taesum-nnaccess-upd.png)

Upon running the code you will see the following in the console:

![](./inst/figures/nnaccess-call.png)
...
![](./inst/figures/taesum-exec.png)

The warning message appears as there is no access to Novo Nordisk output metadata and should be ignored.

In the `stats/output` folder the following should now have appeared:

![](./inst/figures/taesum-output.png)

The `taesumontsas.txt` contains the table produced by the `taesum.R` program:

![](./inst/figures/taesum-table.png)
...
![](./inst/figures/prog-name-output.png)
